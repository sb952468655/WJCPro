...
(function() {
	...
	(function() {
		...
		var dragVar = {};
		/*mask、window*/
		var maskLevel = document.getElementById('maskLevel');
		var maskIframe = document.getElementById('maskIframe');
		var ym_win = document.getElementById('ym-window');
		/*header*/
		var ym_headbox = document.getElementById('ym-tl');
		var ym_head = ym_headbox.firstChild.firstChild;
		var ym_hText = ym_head.firstChild;
		var ym_hTool = ym_hText.nextSibling;
		/*content*/
		var ym_body = document.getElementById('ym-ml').firstChild.firstChild.firstChild;
		/*button*/
		var ym_btn = document.getElementById('ym-btnl');
		var ym_btnContent = ym_btn.firstChild.firstChild;
		/*bottom*/
		var ym_bottom = document.getElementById('ym-bl');
		/*绑定事件*/
		...
		var keydownEvent=function(e) {
			...
			if(btnCache){
				...
				if (keyCode == 9 || keyCode == 39) nofocus&&(btnIndex=-1),document.getElementById(btnCache[++btnIndex == l ? (--btnIndex) : btnIndex].id).focus();
				if (keyCode == 37) nofocus&&(btnIndex=l),document.getElementById(btnCache[--btnIndex < 0 ? (++btnIndex) : btnIndex].id).focus();
		...
		var winVisible = function(visible) {
			...
			btnCache && document.getElementById(btnCache[btnIndex = 0].id).focus(); //第一个按钮获取焦点
		}; //初始化
		var isShow=false;
		var init = function() {
			maskVisible();
		...
		ymPrompt.apply(ymPrompt, {
			...
			getButtons:function(){
				...
				while(btn=btns.shift())rBtn.push(document.getElementById(btn.id));
