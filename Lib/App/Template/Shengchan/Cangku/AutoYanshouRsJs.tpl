...
$(function(){
	...
	$('[name="autoYanshou[]"]').click(function(){
		...
		$('[name="proCode"]',gNull).val($('[name="textBox"]',g).val());

		$('[name="productId[]"]',trNull).val($('[name="productId[]"]',tr).val());
		$('[name="pihao[]"]',trNull).val($('[name="tlGanghao[]"]',tr).val());
		$('[name="proName[]"]',trNull).val($('[name="proName[]"]',tr).val());
	...
	//缸号改变后回影响投入缸号的改变
	/*$('[name="pihao[]"]','#table_else').change(function(){
