	...
	*/
	$('#touliaoAuto').click(function(){
		...
		if(hasTl.length>0 && confirm("是否删除已录入的投料记录?")){
			_autoAddRow(1);
			var trs2 = $(hasTl).parents('.trRow');
			
			if(!$('[name="id"]',trs2).val()>0){
				trs2.remove();
			}else{
				$('[name="btnRemove"]',trs2).css({'color':'red'});
				$('[name="btnRemove"]',trs2).attr('title','如果需要删除，请手动删除');
				window.parent.showMsg("已保存过的数据不支持自动删除，请手动点击删除按钮！",8000);
			}
		}
