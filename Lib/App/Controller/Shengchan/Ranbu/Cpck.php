...
class Controller_Shengchan_Ranbu_Cpck extends Controller_Shengchan_Chuku {
	var $fldMain;
	var $headSon;
	var $rules; //表单元素的验证规则
	// /构造函数
	function __construct() {
		...
		$this->headSon = array(
			...
			'productId' => array(
				...
				"type" => "btPopup",
				'name' => 'productId[]',
				'url'=>url('jichu_chanpin','PopupRanbu'),
				'textFld'=>'proCode',
				'hiddenFld'=>'productId',
				'inTable'=>true,
				'dialogWidth'=>900
		...
		//编辑界面的权限id
		$this->_addCheck = '3-2-2-6';
		$this->_rightCheck = '3-2-2-7';
		$this->_removeCheck = '3-2-2-6';
	}

	function actionShenheList(){
		//权限判断
		$this->authCheck('3-2-2-50');
	...
	*/
	function actionListView(){
		...
		$sql="select x.*,b.kuweiName,
			...
			y.pihao,y.color,y.dengji,y.cnt,y.cntM,y.cntJian,y.memoView,y.id as chuku2ProId,y.return4id,c.compName as clientName,
			y.danjia,y.money
			from cangku_chuku x 
			inner join cangku_chuku_son y on x.id=y.chukuId
			left join jichu_product z on z.id=y.productId
	...
	*/
	function actionEdit(){
		...
		if($chuku2proId!=''){
			...
			$res=$this->_modelExample->findBySql($sql);
			if($res[0]['id']>0){
				$madanCk=&FLEA::getSingleton('Controller_Shengchan_Ranbu_CkWithMadan');
				// dump($madanCk);exit;
				$_GET['controller']="Shengchan_Ranbu_CkWithMadan";
