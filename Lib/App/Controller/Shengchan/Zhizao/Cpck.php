...
class Controller_Shengchan_Zhizao_Cpck extends Controller_Shengchan_Chuku {
	var $fldMain;
	var $headSon;
	var $rules; //表单元素的验证规则
	// /构造函数
	function __construct() {
		...
		$this->headSon = array(
			...
			// 'cntHaveck'=>array('type'=>'bttext',"title"=>'已出库(Kg)','name'=>'cntHaveck[]','readonly'=>true),
			...
			// ***************如何处理hidden?
			'id' => array('type' => 'bthidden', 'name' => 'id[]', "title" => 'id'),
			'unit' => array('type' => 'bthidden', 'name' => 'unit[]', "title" => '单位'),
			// 'ord2proId' => array('type' => 'bthidden', 'name' => 'ord2proId[]', "title" => '花型id'),
		...
		//编辑界面的权限id
		$this->_addCheck = '3-2-1-6';
		$this->_rightCheck = '3-2-1-7';
		$this->_removeCheck = '3-2-1-6';
	}

	function actionShenheList(){
		//权限判断
		$this->authCheck('3-2-1-50');
	...
	*/
	function actionListView(){
		$this->authCheck('3-2-1-49');
		$isdanjiaCheck = $this->authCheck('3-2-2-49-4',true);
		FLEA::loadClass('TMIS_Pager'); 
		$arr = TMIS_Pager::getParamArray(array(
		...
		$sql="select x.*,b.kuweiName,
			...
			y.ganghao,y.color,y.dengji,y.cnt,y.cntM,y.cntJian,y.memoView,y.id as chuku2ProId,y.return4id,c.compName as clientName,y.danjia,y.money
			from cangku_chuku x 
			inner join cangku_chuku_son y on x.id=y.chukuId
			left join jichu_product z on z.id=y.productId
	...
	*/
	function actionRight(){
		...
		if (count($rowset) > 0) foreach($rowset as &$v) {
			...
			$temp = $this->_modelExample->findBySql($sql);
			if($temp[0]['cnt']>0){
				$v['_edit'] = "<span title='已过账，禁止操作'>修改  删除</span>";
		...
		$rowset[]=$this->getHeji($rowset,array('cnt','tuiCnt','money','cntJian','cntM'),'_edit');
	...
	function _beforeDisplayEdit(&$smarty) {
		...
		//查找客户信息
		// $sql="select compName from jichu_client where id='{$areaMain['fld']['clientId']['value']}'";
	...
	*/
	function actionEdit(){
		...
		if($chuku2proId!=''){
			...
			$res=$this->_modelExample->findBySql($sql);
			if($res[0]['id']>0){
				$madanCk=&FLEA::getSingleton('Controller_Shengchan_Zhizao_CkWithMadan');
				$_GET['controller']="Shengchan_Zhizao_CkWithMadan";
