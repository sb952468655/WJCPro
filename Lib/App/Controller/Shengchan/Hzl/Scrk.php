...
class Controller_Shengchan_Hzl_Scrk extends Controller_Shengchan_Ruku {
	// /构造函数
	function __construct() {
		parent::__construct();
		$this->chukuModel = &FLEA::getSingleton('Model_Shengchan_Cangku_Chuku');
		// $this->chuku2Model = &FLEA::getSingleton('Model_Shengchan_Cangku_Chuku2Product');
		$this->_type = '后整理';
		$this->_kind="验收";
		$this->_tuiKind="验收退回";
		$this->_kindLl="";
		...
		$this->fldMain = array(
			...
			'type' => array('title' => '整理厂类型', 'type' => 'select', 'options' => $this->getHzlKind()),
			'jiagonghuId' => array('title' => '加工户', 'type' => 'select', 'model' => 'Model_Jichu_Jiagonghu','condition'=>'isSupplier=0'),
			'ylType' => array('title' => '原料类型', 'type' => 'select', 'options' => array_merge(array(
				array('text'=>'坯布','value'=>'坯布'),
				array('text'=>'染色布','value'=>'染色布')
			),$this->getHzlKind())),
			'kuweiIdLl' => array(
				'title' => '领料仓库',
				"type" => "Popup",
		...
		$this->qitaSon = array(
			...
			'productId' => array(
				...
				"type" => "btPopup",
				'name' => 'productId[]',
				'url'=>url('jichu_chanpin','PopupHzl',array('isKucun'=>0)),
				'textFld'=>'proCode',
				'hiddenFld'=>'productId',
				'inTable'=>true,
				'dialogWidth'=>900
		...
		$this->sonTpl=array(
			...
			"Shengchan/Cangku/jgKindSel.tpl",
			"Shengchan/Cangku/AutoYanshouHzlJs.tpl",
			"Shengchan/Cangku/CopyEdit.tpl",
		);
		$this->_rightCheck='3-2-3-2';
		$this->_addCheck='3-2-3-1';
	}
	...
	function actionRight(){
		...
		$sql="select x.*,a.compName,b.kuweiName,y.return4id
			from cangku_ruku x 
		...
		$sql.=" and (x.kind like '%{$this->_kind}' or x.kind like '%{$this->_tuiKind}')";
		$sql.=" and x.hzl='1'";
		...
		if($arr['typeHzl']!='')$sql.=" and x.type = '{$arr['typeHzl']}'";
	...
	*/
	...
	/**
	 * 处理加工类别，可选可输形式
	 * Time：2014/07/29 13:57:01
	 * @author li
	 * @param Array
	 * @return Array
	*/
	function setOptionsAuto(){
		// dump($this->fldMain['type']);exit;
		$arr=array('印花','水洗','定型');
		$options=array();
		// dump($arr);exit;
		//查找其他值
		$sql="select distinct type from cangku_ruku where isHzl=1";
		$temp = $this->_modelExample->findBySql($sql);
		$temp = array_col_values($temp,'type');

		$arr = array_unique(array_merge($arr,$temp));
		// dump($arr);exit;
		//形成数组
		foreach ($arr as $k => & $v) {
			$options[]=array('text'=>$v,'value'=>$v);
		}

		$this->fldMain['type']['options']=$options;
	}
		...
		*/
		...
		if(!$id) {
			...
			*/
			...
			$lingliao['type']=$_POST['ylType'];
			$lingliao['kind']=$_POST['ylType'].'领用';
			//不是后整理布库存，
			// $lingliao['hzl']=0;
	...
	*/
	function actionSunhaoTongji(){
		...
		$sql.=" group by x.plan2proId,y.jiagonghuId,y.type,x.ganghao order by n.clientId,z.id desc,y.id desc,x.id";
		// dump($sql);exit;
		$pager = &new TMIS_Pager($sql);
		$rowset = $pager->findAll();
	...
	*/
	function actionGetScrkData(){
		...
		$sql.=" and x.kind like '%{$this->_kind}'";
		$sql.=" and x.hzl='1'";
	...
	*/
	function actionTuiAdd(){
		...
		$data=array(
			...
			'type'=>$parent['Rk']['type'],
			'kind'=>$parent['Rk']['type'].$this->_tuiKind,
